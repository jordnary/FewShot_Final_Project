...
mkdir -p "$PROJECT_ROOT/05_paper_reproduction/logs"
mkdir -p "$PROJECT_ROOT/05_paper_reproduction/results"
...
run_one() {
  ...
  python "$PROJECT_ROOT/04_baseline_experiments/run_libfewshot_config_cloud.py" \
    "$PROJECT_ROOT/05_paper_reproduction/configs/$config" \
    2>&1 | tee "$PROJECT_ROOT/05_paper_reproduction/logs/${log_name}_console.log"
}
...
case "$MODE" in
  ...
  all)
    ...
    ;;
  *)
    echo "Usage: bash 05_paper_reproduction/run_frofa_cub_cloud.sh [joint|frozen|all]" >&2
    exit 2
    ;;
esac

python "$PROJECT_ROOT/05_paper_reproduction/summarize_frofa_results.py"
