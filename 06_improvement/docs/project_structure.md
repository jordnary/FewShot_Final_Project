...
`results/*.csv` 保存可追踪的实验结果。当前核心结果包括 global feature linear probe、patch-token MAP paired evaluation 和最终 Stage 6 汇总。
...
`extract_clip_patch_tokens.py` 使用 frozen CLIP ViT-B/16 提取最终层 patch tokens，默认输出 test split 的 `N x 196 x 512` token 缓存。
...
| Paperlike sweep | `run_frofa_map_sweep.py` | `results/clip_vit_b16_postln_patch_frofa_map_sweep.csv` | val sweep 后 test 汇报 |
...
6. 对比 no-FroFA / FroFA，或 MAP / FroFA + MAP。
7. 将核心结果汇总到 `results/final_stage6_summary.csv`，并在 `docs/experiment_summary.md` 中解释。
