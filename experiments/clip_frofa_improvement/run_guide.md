...
本文档面向需要复现CLIP-FroFA 改进实验的人员，默认从项目根目录 `FewShot_Final_Project/` 开始执行命令。
...
├─ data/CUB_200_2011/
└─ experiments/clip_frofa_improvement/
```
...
```text
artifacts/pretrained/open_clip_model.safetensors
```
...
```bash
bash scripts/clip_frofa/run_clip_frofa_cub_cloud.sh
```
...
```text
artifacts/logs/clip_frofa/extract_clip_vit_b16_patch_tokens.log
artifacts/logs/clip_frofa/clip_vit_b16_patch_frofa_map_eval.log
artifacts/features/clip_frofa/cub_test_clip_vit_b16_patch_tokens.npz
experiments/clip_frofa_improvement/results/clip_vit_b16_patch_frofa_map_cub.csv
```
...
```text
artifacts/features/clip_frofa/cub_val_clip_vit_b16_postln_patch_tokens.npz
artifacts/features/clip_frofa/cub_test_clip_vit_b16_postln_patch_tokens.npz
experiments/clip_frofa_improvement/results/clip_vit_b16_postln_patch_frofa_map_sweep.csv
artifacts/logs/clip_frofa/clip_vit_b16_postln_patch_frofa_map_sweep.log
```
...
```bash
python scripts/clip_frofa/extract_clip_features.py \
...
```bash
python scripts/clip_frofa/run_frofa_linear_eval.py \
  --feature-file artifacts/features/clip_frofa/cub_test_clip_vit_b16.npz \
  --output-prefix experiments/clip_frofa_improvement/results/clip_vit_b16_frofa_linear_cub \
  --episodes 1000 \
  --way 5 \
  --shots 1 5 \
  --query 15 \
...
python scripts/clip_frofa/run_frofa_map_eval.py \
  ...
  --output-prefix experiments/clip_frofa_improvement/results/clip_vit_b16_patch_frofa_map_cub_paired \
  --episodes 600 \
  --way 5 \
  --shots 1 5 \
  --query 15 \
...
`artifacts/features/clip_frofa/` 下的 `.npz` 是可复用特征缓存，文件较大，通常作为本地或云端运行产物保留。
...
```bash
tmux new -s clip-frofa-improvement
bash scripts/clip_frofa/run_clip_frofa_cub_cloud.sh
```
...
```bash
tmux attach -t clip-frofa-improvement
```
