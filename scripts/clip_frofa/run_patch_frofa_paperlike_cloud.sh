...
python scripts/clip_frofa/extract_clip_patch_tokens.py \
  ...
  --token-stage post_ln \
  2>&1 | tee artifacts/logs/clip_frofa/extract_clip_vit_b16_postln_patch_tokens.log

PYTHONUNBUFFERED=1 python -u scripts/clip_frofa/run_frofa_map_sweep.py \
  --val-token-file artifacts/features/clip_frofa/cub_val_clip_vit_b16_postln_patch_tokens.npz \
  --test-token-file artifacts/features/clip_frofa/cub_test_clip_vit_b16_postln_patch_tokens.npz \
  --output-prefix experiments/clip_frofa_improvement/results/clip_vit_b16_postln_patch_frofa_map_sweep \
  --episodes 120 \
  --test-episodes 600 \
  --shots 1 5 \
  ...
  --log-interval 20 \
  --no-md \
  2>&1 | tee artifacts/logs/clip_frofa/clip_vit_b16_postln_patch_frofa_map_sweep.log
