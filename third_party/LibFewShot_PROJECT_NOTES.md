...
- `data/` 提供 CUB 等数据和 split 文件。
- `experiments/baseline/` 使用本目录中的 LibFewShot 入口运行 ProtoNet baseline。
- `experiments/frofa_reproduction/` 使用 `FroFAProtoNet` 相关配置复现 FroFA 风格方法。
- `experiments/clip_frofa_improvement/` 在 LibFewShot 复现链路之外补充 CLIP frozen feature、patch-token MAP 和 FroFA 改进实验。

## 使用建议

如果只是运行实验，优先从 `experiments/baseline/`、`experiments/frofa_reproduction/` 或 `experiments/clip_frofa_improvement/` 的脚本进入；如果需要修改 few-shot 方法本身，再进入 `LibFewShot/core/model/` 和 `LibFewShot/config/`。
